...
CITY_TO_REGION = {
    ...
    'Долинська': 'Кіровоградська обл.',
    # Райони Кривого Рогу (для контексту "Кривий Ріг:")
    'Саксагань': 'Дніпропетровська обл.',
    'Довгинцеве': 'Дніпропетровська обл.',
    'Соцмісто': 'Дніпропетровська обл.',
    'Арселор': 'Дніпропетровська обл.',
    'Зарічний': 'Дніпропетровська обл.',
    'Новомайське': 'Дніпропетровська обл.',
}
...
def fix_city_case(city):
    ...
    # Нормалізуємо для пошуку (перша літера велика)
    city_normalized = city[0].upper() + city[1:].lower() if len(city) > 1 else city.upper()
    ...
    # Шукаємо в словнику (з нормалізованим ключем)
    if city_normalized in known_forms:
        return known_forms[city_normalized]
    # Також перевіряємо оригінал на випадок складних форм
