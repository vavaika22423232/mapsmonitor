...
REGION_MAP = {
    ...
    'Дніпропетровська область': 'Дніпропетровська обл.',
    ...
    'Кіровоградська область': 'Кіровоградська обл.'
}
...
CITY_TO_REGION = {
    ...
    'Миколаївський': 'Миколаївська обл.',  # Миколаївський район
    ...
    'Шостка': 'Сумська обл.',
    'Охтирка': 'Сумська обл.',
    'Охтирський': 'Сумська обл.',  # Охтирський район
    ...
    'Новомиргород': 'Кіровоградська обл.',
    'Пантаївка': 'Кіровоградська обл.',
    'Вишеньки': 'Київська обл.',
    'Новопокровка': 'Харківська обл.',  # Харківська область, не плутати з іншими
    ...
    'Дослідне': 'Дніпропетровська обл.',
    'Дмитрівка': 'Дніпропетровська обл.',
    'Дмитрівку': 'Дніпропетровська обл.',
    'Просяна': 'Дніпропетровська обл.',
    ...
    'Золочів': 'Харківська обл.',  # Харківська область, біля фронту (не Львівська!)
    'Золочева': 'Харківська обл.',
    'Золочеві': 'Харківська обл.',
    'Слобідське': 'Харківська обл.',  # Харківська область (не Дніпропетровська!)
    # Одеська область - Нова Миколаївка та інші
    'Нова Миколаївка': 'Одеська обл.',
    'Нову Миколаївку': 'Одеська обл.',
    # Спеціальні локації
...
async def split_cities(city_string):
    ...
    Повертає останнє місто (кінцевий пункт)
    """
    ...
    if '/' in city_string:
        ...
        # Беремо останнє місто (кінцевий пункт)
        city = cities[-1].strip()
...
def normalize_region(region):
    """
    Нормалізує назву області до формату 'Область обл.'
    Виправляє дублювання 'обл обл.' та інші проблеми.
    """
    if not region:
        return region
    ...
    # Виправляємо "Обл обл." -> "обл."
    region = re.sub(r'\s+[Оо]бл\.?\s+обл\.?', ' обл.', region)
    region = re.sub(r'\s+обл\.?\s+обл\.?', ' обл.', region)
    
    # Видаляємо "область" якщо є "обл."
    if 'область' in region.lower() and 'обл.' in region.lower():
        region = region.replace(' область', '').replace(' Область', '')
    
    # Замінюємо "область" на "обл."
    region = region.replace(' область', ' обл.').replace(' Область', ' обл.')
    
    # Якщо немає "обл." в кінці - додаємо
    ...
    # Якщо закінчується на "обл" без крапки - додаємо крапку
...
def fix_city_case(city):
    ...
    # Видаляємо emoji та спецсимволи з назви міста (залишаємо тільки літери, пробіли, дефіси, апострофи)
    ...
    # 🆕 Правила для двослівних назв з "Нову/Стару/Велику/Малу" на початку
    # "Нову Миколаївка" -> "Нова Миколаївка", "Стару Каховку" -> "Стара Каховка"
    words = city.split()
    if len(words) == 2:
        first_word = words[0]
        second_word = words[1]
        
        # Виправляємо перше слово (Нову->Нова, Стару->Стара, Велику->Велика, Малу->Мала)
        first_word_fixed = first_word
        if first_word.lower() == 'нову':
            first_word_fixed = 'Нова'
        elif first_word.lower() == 'стару':
            first_word_fixed = 'Стара'
        elif first_word.lower() == 'велику':
            first_word_fixed = 'Велика'
        elif first_word.lower() == 'малу':
            first_word_fixed = 'Мала'
        
        # Виправляємо друге слово (якщо потрібно)
        second_word_fixed = second_word
        if second_word.endswith('ку') and len(second_word) > 3:
            second_word_fixed = second_word[:-1] + 'а'
        elif second_word.endswith('ну') and len(second_word) > 3:
            second_word_fixed = second_word[:-1] + 'а'
        elif second_word.endswith('гу') and len(second_word) > 3:
            second_word_fixed = second_word[:-1] + 'а'
        
        if first_word_fixed != first_word or second_word_fixed != second_word:
            return f"{first_word_fixed} {second_word_fixed}"
...
def validate_city_region(city, region):
    """
    ...
    # Перевіряємо чи місто є в словнику з іншою областю
    correct_region = CITY_TO_REGION.get(city)
...
def format_bpla_message(city, region, prefix="БПЛА"):
    """
    Формує стандартне повідомлення про БПЛА.
    Нормалізує місто та область, виправляє помилки.
    """
    if not city or not region:
        return None
    
    # Видаляємо зайві символи з міста
    city = city.strip()
    city = re.sub(r'\*+', '', city)  # Видаляємо зірочки **
    city = re.sub(r'[^\w\s\'\-]', '', city, flags=re.UNICODE)  # Видаляємо emoji (залишаємо літери, пробіли, дефіси, апострофи)
    city = re.sub(r'^Район\s+', '', city, flags=re.IGNORECASE)  # "Район Новопокровка" -> "Новопокровка"
    city = re.sub(r'\s+р-н$', ' район', city, flags=re.IGNORECASE)  # "Миколаївський р-н" -> "Миколаївський район"
    city = re.sub(r'\s+р$', ' район', city)  # "Охтирський р" -> "Охтирський район"
    city = city.strip()  # Ще раз trim після видалення emoji
    
    # Видаляємо подвійні дужки з областю типу "(Донеччина)"
    city = re.sub(r'\s*\([^)]*(?:щина|ччина|область|обл\.?)[^)]*\)\s*$', '', city, flags=re.IGNORECASE)
    
    # Нормалізуємо область
    region = normalize_region(region)
    
    # Перевіряємо чи область відповідає місту
    city, region = validate_city_region(city, region)
    
    return f"{prefix} {city} ({region})"
...
async def parse_and_split_message(text, channel_name=None):
    ...
    if vybukhy_match:
        ...
        # Очищаємо location від подвійних дужок з областю
        location = re.sub(r'\s*\([^)]*(?:щина|ччина|область|обл\.?)[^)]*\)\s*$', '', location, flags=re.IGNORECASE)
        location = re.sub(r'\*+', '', location)  # Видаляємо зірочки
        location = re.sub(r'\s+р-н$', ' район', location)  # р-н -> район
        region = normalize_region(region)
    ...
    if vybukhy_no_region_match:
        ...
        city = re.sub(r'\*+', '', city)  # Видаляємо зірочки
        city = re.sub(r'\s+р-н$', ' район', city)  # р-н -> район
        if city:
    ...
    for i, line in enumerate(lines):
        ...
        # Формат заголовка: "✈️Дніпропетровщина:" або "🛵Харківщина:" або "Дніпропетровська область:" - область з/без emoji і двокрапкою
        # Видаляємо всі emoji та спецсимволи на початку рядка
        clean_line = re.sub(r'^[✈️🛵🛸⚠️❗️🔴📡\s]+', '', line).strip()
        ...
        # або "Дніпропетровська область: 1х шахед на Дмитрівку"
        # Область з двокрапкою і списком міст в одному рядку
        region_with_cities_match = re.match(r'^(\S+(?:\s+область)?):\s+(.+)$', clean_line, re.IGNORECASE)
        if region_with_cities_match:
            region_name = region_with_cities_match.group(1).strip()
            ...
            if region:
                ...
                for entry in city_entries:
                    ...
                    # Формат: "1х шахед на Дмитрівку" або "2х шахеди на Суданівку" - шахед/шахеди на місто
                    shahed_na_match = re.match(r'^(\d+)\s*х?\s*шахед[иів]*\s+на\s+(\S+)\.?$', entry, re.IGNORECASE)
                    if shahed_na_match:
                        city = shahed_na_match.group(2).strip().rstrip('.,;')
                        city = fix_city_case(city)
                        city = city[0].upper() + city[1:] if city else city
                        msg = f"БПЛА {city} ({region})"
                        if msg not in messages:
                    ...
                    # Формат: "1 Новомиргород / район" - місто з "/ район"
                    city_rayon_match = re.match(r'^(\d+)\s+(\S+)\s*/\s*район\.?$', entry, re.IGNORECASE)
                    if city_rayon_match:
                        city = city_rayon_match.group(2).strip().rstrip('.,;')
                        city = fix_city_case(city)
                        city = city[0].upper() + city[1:] if city else city
                        msg = f"БПЛА {city} ({region})"
                        if msg not in messages:
                    ...
                    # Формат: "1 навпроти міста над Дніпром, в бік Канева" - беремо місто з "в бік City"
                    v_bik_match = re.search(r'в\s+бік\s+(\S+)$', entry, re.IGNORECASE)
                    if v_bik_match:
                        city = v_bik_match.group(1).strip().rstrip('.,;')
                        city = fix_city_case(city)
                        city = city[0].upper() + city[1:] if city else city
                        msg = f"БПЛА {city} ({region})"
                        if msg not in messages:
        ...
        # Формат: "N шахеди/шахедів на X з Y" або "N шахеди на X" або "1х шахед на X"
        shahedy_na_match = re.match(r'^\d+\s*х?\s*шахед[іиів]*\s+на\s+(\S+)(?:\s+з\s+.+)?\.?$', line, re.IGNORECASE)
        if shahedy_na_match:
            city = shahedy_na_match.group(1).strip().rstrip('.,;')
            city = fix_city_case(city)
            city = city[0].upper() + city[1:] if city else city
            region = current_region
            if not region:
        ...
        # Формат: "3х БпЛА в районі Сновська" - з "х" і типом БПЛА
        v_rayoni_x_match = re.match(r'^(\d+)\s*х?\s*(БпЛА|БПЛА)\s+(?:в|у)\s+район[іу]?\s+(\S+)$', line, re.IGNORECASE)
        if v_rayoni_x_match and current_region:
            city = v_rayoni_x_match.group(3).strip()
            city = fix_city_case(city)
            city = city[0].upper() + city[1:] if city else city
            msg = f"БПЛА {city} ({current_region})"
            messages.append(msg)
        ...
        if ready_match:
            logger.info(f"🔍 ready_match: line='{line}', city='{ready_match.group(1)}', region='{ready_match.group(2)}'")
            ...
            if 'вибух' in next_line.lower() or 'вибух' in line.lower():
                logger.info(f"🔍 Пропуск через 'вибух': {line}")
            ...
            city = re.sub(r'\s+з\s+(?:Сумщини|Чернігівщини|Полтавщини|Черкащини|Київщини|Київщині|Харківщини|Дніпропетровщини|Миколаївщини|Одещини|Херсонщини|Запоріжжя|Донеччини|Луганщини|Житомирщини|Вінниччини|Хмельниччини|Рівненщини|Волині|Львівщини|Тернопільщини|Івано-Франківщини|Закарпаття|Кіровоградщини)\s*$', '', city, flags=re.IGNORECASE).strip()
            
            # Видаляємо "Шахеди на/через", "на/через", "в бік X" конструкції
            city = re.sub(r'^[Шш]ахед[іиів]*\s+(?:на/через|на|через)\s+', '', city, flags=re.IGNORECASE).strip()
            city = re.sub(r'\s+(?:в\s+бік|у\s+бік|на)\s+\S+\s*$', '', city, flags=re.IGNORECASE).strip()
            ...
            skip_words = ['кружляють', 'кружляє', 'летить', 'летять', 'рухається', 'рухаються', 
                         ...
                         'шахеди', 'шахед', 'на/через', 'в бік', 'у бік']
            # Пропускаємо напрямки (курси), аббревіатури та російські міста
            skip_cities = ['західний', 'східний', 'північний', 'південний', 
                          'захід', 'схід', 'північ', 'південь',
                          'чс', 'сау', 'зсу', 'ппо',
                          'ахтарск', 'таганрог', 'ростов', 'бєлгород', 'курськ']
            should_skip = False
            if city.lower() in skip_cities:
                should_skip = True
            ...
            if should_skip or len(city) < 2:
                logger.info(f"🔍 Пропуск: should_skip={should_skip}, len(city)={len(city)}, city='{city}'")
                continue
            # Виправляємо відмінок
            city = fix_city_case(city)
            # Capitalize першу літеру міста
    ...
    # Фінальна фільтрація та виправлення повідомлень
    logger.info(f"🔍 Повідомлення до фільтрації: {messages}")
    filtered_messages = []
    for msg in messages:
        # Якщо повідомлення вже в правильному форматі з областю - перевіряємо тільки skip_words
        has_region = '(' in msg and ')' in msg and 'обл' in msg.lower()
        
        # Очищаємо назву міста від сміття
        # "БПЛА Шахеди на/через Васильківку в бік Самару (обл)" -> "БПЛА Васильківка (обл)" або "БПЛА Самар (обл)"
        clean_match = re.match(r'^(БПЛА|Ракета|КАБ)\s+(.+?)\s*\((.+?)\)$', msg)
        if clean_match:
            threat_type = clean_match.group(1)
            city_part = clean_match.group(2)
            region = clean_match.group(3)
            
            # Видаляємо "Шахеди на/через", "на/через" на початку
            city_part = re.sub(r'^[Шш]ахед[іиів]*\s+(?:на/через|на|через)\s+', '', city_part).strip()
            city_part = re.sub(r'^(?:на/через|на|через)\s+', '', city_part).strip()
            
            # Якщо є "в бік X" - беремо X як кінцевий пункт
            v_bik_match = re.search(r'[ву]\s+бік\s+(\S+)', city_part, re.IGNORECASE)
            if v_bik_match:
                city_part = v_bik_match.group(1).strip()
            else:
                # Видаляємо "в бік X" якщо є
                city_part = re.sub(r'\s+[ву]\s+бік\s+\S+', '', city_part).strip()
            
            # Видаляємо "з Області" конструкцію (напр. "Народичі з Київщині" -> "Народичі")
            city_part = re.sub(r'\s+з\s+(?:Сумщини|Чернігівщини|Полтавщини|Черкащини|Київщини|Київщині|Харківщини|Дніпропетровщини|Миколаївщини|Одещини|Херсонщини|Запоріжжя|Донеччини|Луганщини|Житомирщини|Вінниччини|Хмельниччини|Рівненщини|Волині|Львівщини|Тернопільщини|Івано-Франківщини|Закарпаття|Кіровоградщини|\S+щини|\S+ччини)\s*$', '', city_part, flags=re.IGNORECASE).strip()
            
            # Видаляємо інші "з X" конструкції
            city_part = re.sub(r'\s+з\s+\S+$', '', city_part).strip()
            
            # Беремо перше/останнє слово якщо залишилось багато
            if ' ' in city_part and len(city_part.split()) > 2:
                # Якщо забагато слів - беремо останнє (кінцевий пункт)
                city_part = city_part.split()[-1]
            
            city_part = fix_city_case(city_part)
            city_part = city_part[0].upper() + city_part[1:] if city_part else city_part
            
            # 🆕 Валідуємо що місто в правильній області
            city_part, region = validate_city_region(city_part, region)
        ...
        if city_check:
            ...
            if len(city_name) < 3:
                continue
            # Пропускаємо якщо "місто" насправді є назвою області
            region_names = ['харківщина', 'сумщина', 'полтавщина', 'чернігівщина', 'київщина',
                           'житомирщина', 'дніпропетровщина', 'черкащина', 'запоріжжя', 'донеччина',
                           'луганщина', 'миколаївщина', 'херсонщина', 'одещина', 'кіровоградщина',
                           'вінниччина', 'вінничина', 'хмельниччина', 'закарпаття', 'львівщина',
                           'рівненщина', 'волинь', 'тернопільщина', 'івано-франківщина', 'крим']
            if city_name.lower() in region_names or city_name.lower().rstrip('иіуа') in [r.rstrip('иіуа') for r in region_names]:
                continue
        ...
        # Якщо БПЛА/Ракета без області в дужках - спробуємо додати
        if re.match(r'^(БПЛА|Ракета)\s+(.+)$', msg) and '(' not in msg:
            match = re.match(r'^(БПЛА|Ракета)\s+(.+)$', msg)
            threat_type = match.group(1)
            city = match.group(2).strip()
            # Спробуємо знайти область в словнику
            region = CITY_TO_REGION.get(city, None)
            if not region:
                # Спробуємо з великої літери
                city_cap = city[0].upper() + city[1:] if city else city
                region = CITY_TO_REGION.get(city_cap, None)
            if not region:
                # Спробуємо геокодер синхронно через словник
                region = CITY_TO_REGION.get(fix_city_case(city), None)
            if region:
                msg = f"{threat_type} {city} ({region})"
            else:
                # Пропускаємо якщо не можемо визначити область
                logger.info(f"🔍 Пропуск через відсутність області: {msg}")
                continue
