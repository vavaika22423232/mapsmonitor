...
async def parse_and_split_message(text):
    ...
    # Пропускаємо повідомлення про загрозу обстрілу, укриття тощо (весь текст)
    if re.search(r'загроза\s+обстрілу|перейдіть\s+в\s+укриття|прямуйте\s+в\s+укриття', text, re.IGNORECASE):
        return []
