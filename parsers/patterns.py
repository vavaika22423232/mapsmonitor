...
THREAT_TYPE = PatternGroup({
    'recon': _compile(r'(?:розвідк|розвідувальн|розвідувальний\s+бпла|бпла-розвідник)', _FLAGS),
...
LOCATION = PatternGroup({
    ...
    # Format: "X курсом на City (Region обл.)" - recon/BPLA with course
    'kursom_na_city_region': _compile(
        r'курсом\s+на\s+(.+?)\s*\(([^)]+обл\.?)\)'
    ),
    ...
    # Format: "курс City - City" (without "на")
    'kurs_city_dash': _compile(
        r'курс(?:ом)?\s+'
        r'([А-ЯІЇЄҐа-яіїєґ\'\-\s]+?)'
        r'(?:[.!?…]|$)'
    ),
    ...
    # Format: "Nx Групи КР курсом на City" or "Група КР курсом на City"
    'grupa_kr_kursom': _compile(
        r'(?:(\d+)\s*х?\s*)?'
        r'[Гг]руп[аи]?\s+КР\s+курсом\s+на\s+'
        r'([А-ЯІЇЄҐа-яіїєґ\'\-\s]+)'
    ),
    ...
    # Format: "✈️ City/р-н - обережно по БПЛА!"
    'oberezhno_bpla': _compile(
        r'^[✈️🛸\s]*'
        r'([А-ЯІЇЄҐа-яіїєґ\'\ʼ\-]+)'
        r'(?:/(?:р-н|район))?\s*'
        r'[-–—]\s*обережно\s+по\s+(?:БпЛА|БПЛА)'
    ),
...
ROCKET = PatternGroup({
    ...
    'vidbiy_ballistyka': _compile(
        r'відбій\s+загроз[иі]\s+(?:застосування\s+)?(?:балістич|балістики)'
    ),
